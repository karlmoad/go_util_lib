...
var (
	...
	dashCommentPattern           = regex.NewPattern(`^--.*`)
	slashCommentPattern          = regex.NewPattern(`^//.*`) //, commentHandler},
	hashCommentPattern           = regex.NewPattern(`^#.*`)
...
func NewGrammarDialect() *AntlrG4 {
	return &AntlrG4{markerQueue: queue.NewLIFOQueue[lexer.TokenKind]()}
}

func (g *AntlrG4) RegisterLexer(reg *lexer.Registry) {
	...
	reg.RegisterTokenizationHandler(lexer.RegexHandler(hashCommentPattern, g.SkipHandler(hashCommentPattern)))
...
func (g *AntlrG4) RegisterParser(reg *parser.Registry) {
...
func (g *AntlrG4) SkipHandler(pattern *regex.Pattern) lexer.TokenizationHandler {
...
func (g *AntlrG4) CommentStateCallback(lex *lexer.Lexer) bool {
...
func (g *AntlrG4) MultilineCommentHandler(lex *lexer.Lexer) (*lexer.Token, bool) {
	if match, valid := multilineCommentOpenPattern.MatchSourceStart(lex.Remainder()); valid {
		lex.Advance(len(match))
...
func (g *AntlrG4) IsNewExpr(p *parser.Parser) bool {
...
func (g *AntlrG4) NewExpressionCallback(p *parser.Parser) bool {
...
func (g *AntlrG4) NewExpressionHandler(p *parser.Parser) (ast.Element, bool, error) {
	if g.IsNewExpr(p) {
...
func (g *AntlrG4) AlternativeHandler(p *parser.Parser) (ast.Element, bool, error) {
...
func (g *AntlrG4) StringOrIdentifierHandler(p *parser.Parser) (ast.Element, bool, error) {
...
func (g *AntlrG4) UnknownStatementHandler(p *parser.Parser) (ast.Element, bool, error) {
	return UnknownStmt{Token: p.Advance(), Pos: p.Pos()}, true, nil
}

func (g *AntlrG4) GroupedExprHandler(open lexer.TokenKind, close lexer.TokenKind, group, optional, repeat bool) parser.ParsingHandler {
